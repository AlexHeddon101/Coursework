...
public String newMessage(@FormParam("username") String username,
    ...
    int userId = Authentication.User.nextId();
    String success = UserService.insert(new Authentication.User(userId, username, password1, token));
    ...
    public String newMessage(@FormParam("username") String username,
        ...
        UserService.selectAllInto(Authentication.User.users);
        for (Authentication.User u: Authentication.User.users) {
            if (u.getUsername().toLowerCase().equals(username.toLowerCase())) {
